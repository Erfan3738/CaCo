...
class CaCo(nn.Module):
    ...
    def __init__(self, base_encoder,args, dim=128, m=0.99):
        ...
        self.encoder_k.fc = nn.Sequential(nn.Linear(dim_mlp, dim_mlp), nn.ReLU(), self.encoder_q.fc)
    ...
    def forward_withoutpred_sym(self,im_q,im_k,moco_momentum):
        q = self.encoder_q(im_q, use_feature=True)  # queries: NxC
        q = nn.functional.normalize(q, dim=1)
        q_pred = q
        k_pred = self.encoder_q(im_k, use_feature=True)  # queries: NxC
